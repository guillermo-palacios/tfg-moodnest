...
\begin{xltabular}{\textwidth}{|>{\columncolor{white}\bfseries}c|X|}
    ...
    RF5.3 & El sistema mostrará en el panel principal un mensaje de bienvenida dinámico determinado por dos parámetros: la franja horaria local (mañana, tarde o noche) y el estado de su registro diario (pendiente de rellenar o ya completado en el día actual). \\ \hline
...
Para realizar la evaluación, se ha utilizado una lista de control (\textit{Checklist}) basada en los criterios de calidad principales del estándar IEEE 830 \cite{ieee830}. Cada requisito se ha revisado individualmente para comprobar que cumple con estos cinco criterios básicos:
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textit{Corregido:} El sistema proporcionará retroalimentación visual en un tiempo máximo de 0,5 segundos ante cualquier acción importante.
    \end{itemize}
\end{itemize}

\textit{Nota: Para mantener la coherencia del documento, las \hyperref[tab:rf_total]{tablas generales de especificación de requisitos} presentadas en los apartados anteriores ya reflejan estas versiones finales y corregidas.}
...
En un modelo relacional (SQL) como PostgreSQL o MySQL, esta flexibilidad obligaría a realizar diseños de tablas complejos con múltiples uniones o el uso extensivo de valores nulos, lo que penalizaría el rendimiento de las consultas (incumpliendo el \hyperref[tab:rnf_total]{RNF2.1}). Por ello, se ha optado por un modelo NoSQL orientado a documentos con MongoDB \cite{bib:mongodb}. Esta tecnología permite almacenar cada registro como un documento JSON \cite{bib:json} independiente, facilitando que la estructura de los datos evolucione de forma natural y que el acceso a la información sea directo y mucho más veloz.
...
Se ha seleccionado Spring Boot \cite{bib:springboot} basado en el lenguaje Java \cite{bib:java} debido a su madurez y a la potencia de su ecosistema. Especialmente el uso de Spring Security, que permite implementar de forma nativa la protección de datos y el acceso mediante tokens JWT exigidos en el \hyperref[tab:rnf_total]{RNF4.2}. Además, la integración con MongoDB a través de Spring Data permite que el desarrollo sea mucho más ágil, garantizando que el backend sea capaz de procesar las peticiones concurrentes definidas en el \hyperref[tab:rnf_total]{RNF2.3} sin degradar la experiencia del usuario.

\subsubsection{Interfaz de Usuario y Visualización de Datos}
La interfaz de la aplicación tiene dos retos principales: la agilidad \hyperref[tab:rnf_total]{(RNF3.2)} y la complejidad de los gráficos estadísticos. Tras analizar opciones como Angular o Vue, se ha determinado que React \cite{bib:react} es la opción más viable para este proyecto, utilizando la biblioteca Chart.js \cite{bib:chartjs} para la generación y gestión de todas las visualizaciones del sistema.

React permite construir la interfaz mediante componentes reutilizables, lo que facilita enormemente el mantenimiento del código. Su capacidad para gestionar el estado de la aplicación de forma reactiva asegura que los cambios en los datos se reflejen instantáneamente en los gráficos generados con Chart.js sin necesidad de recargar la página, cumpliendo así con los tiempos de renderizado exigidos. Asimismo, la integración de esta biblioteca (Chart.js) específica garantiza que la implementación de los gráficos de líneas, barras y circulares sea técnica y operativamente posible.
...
\begin{xltabular}{\textwidth}{|>{\columncolor{gray!20}\bfseries}p{3.5cm}|X|}
    ...
    Descripción & El usuario elimina de forma permanente e irreversible su cuenta y todos los datos asociados a la misma. \\ \hline
    Actor Principal & Usuario Registrado \\ \hline
    Actor Secundario & Ninguno \\ \hline
    Precondición & El usuario debe estar autenticado en el sistema. \\ \hline
    Flujo Principal & 
...
\begin{xltabular}{\textwidth}{|>{\columncolor{gray!20}\bfseries}p{3.5cm}|X|}
    ...
    \begin{enumerate}[leftmargin=0.5cm, topsep=0pt, partopsep=0pt, parsep=0pt, itemsep=4pt]
        ...
        \item El sistema presenta el formulario del registro con sus datos actuales pre-cargados (fecha, puntuación, etiquetas y comentario).
